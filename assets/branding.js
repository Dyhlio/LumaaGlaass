...
;(() => {
  ...
  // Example: window.LumaaGlaassOptions = { homeCarousel: false, collectionFilter: true };
  // homeCarousel: rotating banner or still background. collectionFilter: mixed collection filters.
  // Missing/invalid values use defaults. Save and fully reload after changes.
  // See docs/customization.md for home-carousel and filter-mixed-collections.
  const defaults = { homeCarousel: true, collectionFilter: true };
  const configured = window.LumaaGlaassOptions || {};
  const collectionFilterEnabled = typeof configured.collectionFilter === 'boolean' ? configured.collectionFilter : defaults.collectionFilter;
  ...
  // Collections - Reuse the main scheduler; no separate observer or polling.
  function createCollectionFilter() {
    ...
    style.textContent = `
  /* Collections - Show type selection only for mixed collections. */
  ...
  `;
    ...
    return {
      ...
      stop() { stopped = true; clearCollectionFilter(); style.remove(); }
    };
  }
  const collectionFilterController = collectionFilterEnabled ? createCollectionFilter() : null;
  ...
  const schedule = () => {
    ...
    syncSearchArt();
    collectionFilterController?.sync();
    if (scheduled) return;
  ...
  observer.observe(document.documentElement, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'hidden', 'data-id', 'data-type', 'disabled', 'lang', 'data-culture'] });
  ...
  window[KEY] = { get status() { return { active: !!hero?.isConnected, items: items.length, loading, error: lastError }; }, stop() {
    ...
    if (backgroundRequest) backgroundRequest.image.onload = backgroundRequest.image.onerror = null;
    backgroundRequest = null;
    observer.disconnect(); collectionFilterController?.stop(); listeners.forEach(fn => fn());
