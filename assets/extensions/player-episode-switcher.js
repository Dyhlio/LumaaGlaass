/* LumaaGlaass - Optional in-player episode selection. */
(() => {
    'use strict';
    ...
    // Installation: docs/customization.md#in-player-episode-switcher.
    ...
    const key = '__lumaaGlaassPlayerEpisodeSwitcher';
    window[key]?.stop();
    let manager, episodeButton, opener, dialog, dialogItem, container, stopped = false, busy = false, generation = 0;
    let episodeRequest = 0;
    // Reuse the version dialog's theme classes; scope local layout to this extension.
    const style = document.createElement('style');
    style.textContent = `
      .dialog.lg-player-episodes-dialog { font:inherit; color:var(--aa-text,#f5f5f7); margin:auto; inset:0; width:min(760px,calc(100vw - 32px)); max-width:calc(100% - 32px); max-height:calc(100dvh - 40px); padding:0; box-sizing:border-box; border-radius:24px; overflow:auto; color-scheme:dark; }
      .lg-player-episodes-dialog::backdrop { background:rgba(0,0,0,.55); }
      .dialog.lg-player-episodes-dialog[open] { display:flex; flex-direction:column; overflow:hidden; }
      .lg-player-episodes-dialog header { display:flex; align-items:center; justify-content:space-between; gap:16px; padding:20px; flex-shrink:0; background:transparent; border-bottom:1px solid #ffffff20; }
      .lg-player-episodes-dialog h2 { font-size:20px; margin:0; overflow-wrap:anywhere; }
      ...
      .lg-player-episodes-dialog :focus-visible { outline:2px solid white; outline-offset:2px; }
      .lg-player-episodes-dialog button:disabled { cursor:default; opacity:.45; }
      ...
      @media(max-width:640px) {
        .dialog.lg-player-episodes-dialog { width:calc(100% - 24px); max-width:calc(100% - 24px); max-height:calc(100dvh - 32px); }
        .lg-player-episodes-dialog header,.lg-player-version-body { padding:16px; }
      }
      @media(prefers-reduced-transparency:reduce),(forced-colors:active) { .dialog.lg-player-episodes-dialog { background:Canvas!important; color:CanvasText; backdrop-filter:none!important; } }
    `;
    ...
    function context() {
        ...
        if (!item?.Id || item.Type !== 'Episode' || !item.SeriesId || !state.PlayState?.CanSeek) return null;
        return { pm, player, item, state };
    }

    function label() {
        return translate('Episodes', 'Episodes');
    }
    ...
    async function open() {
        ...
        node.className = 'dialog opened lg-player-episodes-dialog';
        node.classList.add('lg-player-episodes');
        node.style.fontFamily = getComputedStyle(opener).fontFamily;
        const content = document.createElement('div');
        content.className = 'lg-player-version-body';
        const header = document.createElement('header');
        const title = document.createElement('h2');
        title.id = 'lg-player-episode-title';
        ...
        node.showModal();
        try {
            await loadEpisodes(api, ctx, node, list, status, seasons);
    ...
    function sync() {
        ...
        try {
            ...
            if (!ctx) { episodeButton?.remove(); episodeButton = null; if (!busy) close(); return; }
            ...
            if (!episodeButton?.isConnected) {
                ...
                episodeButton.append(icon); episodeButton.onclick = open;
            }
            // Keep Episodes before Versions regardless of extension installation order.
            const target = anchor.parentElement.querySelector('.lg-player-version-button') || anchor;
            if (episodeButton.nextElementSibling !== target) target.before(episodeButton);
            episodeButton.title = label(); episodeButton.setAttribute('aria-label', label()); episodeButton.disabled = busy;
        } catch (error) { console.debug('LumaaGlaass player episode switcher is unavailable.', error); }
    }
    const timer = setInterval(sync, 1000);
    window[key] = { stop() { stopped = true; clearInterval(timer); close(); episodeButton?.remove(); style.remove(); delete window[key]; } };
    sync();
})();
